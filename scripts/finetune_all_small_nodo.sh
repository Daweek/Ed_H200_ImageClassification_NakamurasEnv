...
for FT_DATASET_NAME in cifar10 cifar100 cars flowers voc12 places30 imnet100; do
    if [ "${FT_DATASET_NAME}" = "cifar10" ]; then
        NUM_CLS=10
        TRAIN_IMG=50000
        VAL_IMG=10000
    elif [ "${FT_DATASET_NAME}" = "cifar100" ]; then
        NUM_CLS=100
        TRAIN_IMG=50000
        VAL_IMG=10000
    elif [ "${FT_DATASET_NAME}" = "cars" ]; then
        NUM_CLS=196
        TRAIN_IMG=8144
        VAL_IMG=8041
    elif [ "${FT_DATASET_NAME}" = "flowers" ]; then
        NUM_CLS=102
        TRAIN_IMG=6552
        VAL_IMG=818
    elif [ "${FT_DATASET_NAME}" = "voc12" ]; then
        NUM_CLS=20
        TRAIN_IMG=13609
        VAL_IMG=13841
    elif [ "${FT_DATASET_NAME}" = "places30" ]; then
        NUM_CLS=30
        TRAIN_IMG=149254
        VAL_IMG=3000
    elif [ "${FT_DATASET_NAME}" = "imnet100" ]; then
        NUM_CLS=100
        TRAIN_IMG=128585
        VAL_IMG=5000
    else
        echo "Unknown dataset: ${FT_DATASET_NAME}"
        continue
    fi


    blank_lines 4
    cecho orange "##### START - Dataset ${FT_DATASET_NAME} Iteration $((iter + 1))/4 ##############"
    ...
    export EXPERIMENT=${CURRENT_JOB_ID}_${FT_DATASET_NAME}_${PRT_DATASET}_VAConfig_H_Random
