...
module load hpcx-mt cuda/12.6/12.6.2 cudnn/9.8/9.8.0 nccl/2.24/2.24.3
...
export MASTER_ADDR=$(/usr/sbin/ip a show dev ibp184s0 | grep inet | cut -d " " -f 6 | cut -d "/" -f 1|head -n 1)
...
mpirun -np ${NUM_GPUS} --use-hwthread-cpus --bind-to socket --oversubscribe -x MASTER_ADDR=${MASTER_ADDR} -x MASTER_PORT=${MASTER_PORT} python -B main.py \
    ...
    ckpt=/home/qai10413uh/working/transformer/nakamura/Ed_H200_ImageClassification_NakamurasEnv/checkpoints/fromABCI_tofinetune/pretrain_deit_tiny_va1k_lr1.0e-3_epochs300_bs1024_files_512x_VAconfig_V_last.pth.tar \
    output_dir=./checkpoints/${EXPERIMENT} \
    mode=finetune +script=tar +logwandb=False
