...
FT_DATASET_NAME=imnet
NUM_CLS=1000
TRAIN_IMG=1281167
VAL_IMG=50000

cecho magenta "______Start Untaring_________"
cecho magenta "Job Started on: $(date)"
u_start_time=$(date +%s%3N)

blank_lines 2
cecho green "Copy and Untar..."
# --display-map --display-allocation
mpiexec  -npernode 1 -np 4 -hostfile $PBS_NODEFILE --use-hwthread-cpus --bind-to none --oversubscribe tar -xf /home/qai10413uh/dataset/train.tar -C $SSD
readlink -f ${SSD}/train

mpiexec  -npernode 1 -np 4 -hostfile $PBS_NODEFILE --use-hwthread-cpus --bind-to none --oversubscribe tar -xf /home/qai10413uh/dataset/val.tar -C $SSD
readlink -f ${SSD}/val

cecho green "Finished copying and Untar..."
u_end_time=$(date +%s%3N)
u_total_duration=$((u_end_time - u_start_time))
convert_milliseconds "$total_duration"
cecho magenta "Untar finished on: $(date)"
cecho magenta "______Finish Untaring_________"
echo "                   "
...
for iter in {0..0}; do
    ...
    mpiexec --display-map --display-allocation -npernode ${NUM_GPU_PER_NODE} -np ${NUM_GPUS} -hostfile $PBS_NODEFILE --use-hwthread-cpus --bind-to none --oversubscribe python -B main.py \
        ...
        data.trainset.root=$SSD/train data.valset.root=$SSD/val \
        data.loader.batch_size=64 model=vit model.arch.model_name=deit_tiny_patch16_224 epochs=300  \
        model.optim.opt=adamw model.optim.lr=0.001 model.optim.weight_decay=0.05 \
        model.scheduler.args.warmup_epochs=10 \
        logger.save_epoch_freq=20 \
...
cecho blue "The whole experiments Duration: "
