...
def train_one_epoch(cfg, epoch, dataloader, mixup_fn, model, criterion, optimizer, loss_scaler, lr_scheduler, n_iter):
    ...
    for idx, data in enumerate(trainlogger.log_every(dataloader, n_iter, print_freq, header)):
        ...
        if not cfg.model.scheduler.args.step_per_epoch:
            print("Scheduler on the batch step->>>>>>")
...
def evaluate(dataloader, model, n_iter):
    ...
    evalloagger.add_meter('eval_top1', SmoothedValue(window_size=1, fmt='{value:.3f}'))
    ...
    for data in evalloagger.log_every(dataloader, n_iter, 10, header):
        ...
        evalloagger.update(eval_top1=top1.item(), n=batch_size)

    evalloagger.synchronize_between_processes()
    print('* Acc@1: {top1.global_avg:.3f} Eval loss: {losses.global_avg:.3f}'.format(top1=evalloagger.eval_top1, losses=evalloagger.eval_loss))
...
def main(cfg):
    ...
    # Fix random seed
    if cfg.seed != -1:
        print("Setting the seeds for reproducibility...")
    ...
    for epoch in range(start_epoch, cfg.epochs + 1):
        ...
        if cfg.mode == 'finetune':
            ...
            max_accuracy = max(max_accuracy, eval_stats['eval_top1'])
